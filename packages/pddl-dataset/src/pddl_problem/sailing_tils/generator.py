"""Sailing temporal-PDDL problem generator (TIL variant)."""
...
from .schema import SailingTilsConfig
...
def generate_problem(config: SailingTilsConfig) -> GeneratedProblem:
    ...
    for i, person_name in enumerate(person_names):
        ...
        # px / py kept for parity with the original generator (currently unused
        # but reserved for future absolute-position semantics).
    ...
    init_atoms.append(f"\t\t(at {int(deadline)} (not (still_alive)))")
    init_block = "\n".join(init_atoms)
